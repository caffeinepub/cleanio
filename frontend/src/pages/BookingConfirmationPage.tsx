...
import { CheckCircle, Home, Calendar, ArrowRight, UserCheck, Loader2 } from 'lucide-react';
...
function getCleaningSubOptionLabel(booking: { cleaningSubOption?: { __kind__: string } } | null): string | null {
  if (!booking?.cleaningSubOption) return null;
  if (booking.cleaningSubOption.__kind__ === 'colourFoamWashing') return 'Colour Foam Washing ₹199';
  if (booking.cleaningSubOption.__kind__ === 'normalFoamWashing') return 'Normal Foam Washing ₹149';
  return null;
}
    ...
    <div className="container mx-auto px-4 py-16 max-w-lg">
      <div className="bg-card border border-border rounded-3xl p-8 text-center shadow-card animate-fade-in-up">
        ...
        <p className="text-muted-foreground mb-8">
          Your service has been booked successfully. Our technician will be at your doorstep on time.
        </p>
        ...
        <div className="bg-charcoal-light border border-border rounded-2xl p-5 text-left space-y-3 mb-8">
          <div className="flex items-center justify-between">
            ...
            <span className="text-brand-orange font-mono font-bold text-sm">{bookingId}</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            {isLoading ? (
              ...
              <span className={`text-sm font-medium ${booking?.mechanicName ? 'text-foreground' : 'text-muted-foreground italic'}`}>
        ...
        <div className="bg-brand-orange/10 border border-brand-orange/20 rounded-xl p-4 mb-8 text-left">
          ...
          <p className="text-xs text-muted-foreground">
            Our team will call you to confirm the booking and provide the technician's details. Keep your phone handy!
          </p>
        </div>
