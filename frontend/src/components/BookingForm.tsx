...
import { VehicleType, Capacity, ServiceType, type CleaningSubOption } from '../backend';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createBooking.mutateAsync({
        ...
        cleaningSubOption: buildCleaningSubOption(),
      });
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {showCapacitySelector && (
        <div className="space-y-3">
          <Label className="text-foreground font-semibold">Engine Capacity</Label>
          <RadioGroup
            value={capacity}
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-foreground text-sm">Upto 200cc</p>
                  <p className="text-xs text-muted-foreground">Scooters, small bikes</p>
                </div>
              </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-foreground text-sm">Above 200cc</p>
                  <p className="text-xs text-muted-foreground">Sports, cruiser bikes</p>
                </div>
              </div>
              <span className="text-brand-orange font-black text-lg">₹1199</span>
            </label>
          </RadioGroup>
      ...
      {serviceType === ServiceType.cleaning && cleaningSubOption && cleaningPrice && (
        <div className="flex items-center justify-between p-4 bg-brand-orange/10 border border-brand-orange/30 rounded-xl">
          <div>
            <p className="text-xs text-muted-foreground font-medium mb-0.5">Selected Wash</p>
            <p className="text-sm font-semibold text-foreground">
              {CLEANING_OPTION_LABELS[cleaningSubOption]}
            </p>
          </div>
          <span className="text-brand-orange font-black text-2xl">₹{cleaningPrice}</span>
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        >
          <label className={`flex items-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all flex-1 ${
            vehicleType === VehicleType.scooter ? 'border-brand-orange bg-brand-orange/10' : 'border-border hover:border-brand-orange/50'
          }`}>
            <RadioGroupItem value={VehicleType.scooter} id="scooter" />
            <span className="text-sm font-medium text-foreground">🛵 Scooter</span>
          </label>
          <label className={`flex items-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all flex-1 ${
            vehicleType === VehicleType.motorcycle ? 'border-brand-orange bg-brand-orange/10' : 'border-border hover:border-brand-orange/50'
          }`}>
            <RadioGroupItem value={VehicleType.motorcycle} id="motorcycle" />
            <span className="text-sm font-medium text-foreground">🏍️ Motorcycle</span>
          </label>
        </RadioGroup>
      </div>
      ...
      {/* Address */}
      <div className="space-y-2">
        <Label htmlFor="address" className="text-foreground font-semibold">Doorstep Address</Label>
        <Input
          id="address"
          value={address}
      ...
      {/* Time Slot */}
      <div className="space-y-2">
        <Label className="text-foreground font-semibold">Preferred Time Slot</Label>
        <Select value={timeSlot} onValueChange={setTimeSlot}>
          ...
          <SelectContent className="bg-card border-border">
            {TIME_SLOTS.map((slot) => (
              <SelectItem key={slot} value={slot} className="text-foreground hover:bg-charcoal-light">
                {slot}
              </SelectItem>
            ))}
          </SelectContent>
