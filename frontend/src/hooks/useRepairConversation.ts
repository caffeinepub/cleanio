...
const SEVERITY_OPTIONS = ['Minor – still rideable', 'Moderate – affects performance', 'Severe – cannot ride'];
const DURATION_OPTIONS = ['Just started today', 'Past few days', 'More than a week', 'Ongoing for a while'];
  ...
  const handleReply = (reply: string) => {
    ...
    if (step === 'issue_category') {
      ...
      if (reply === 'Electrical Issue') {
        ...
        setTimeout(() => {
          addBotMessage('Got it! Can you tell me more about the electrical issue?', ELECTRICAL_DETAILS);
        }, 400);
      } else if (reply === 'Brake Problem') {
        setStep('brake_detail');
        setTimeout(() => {
          addBotMessage('Understood. What kind of brake problem are you experiencing?', BRAKE_DETAILS);
        }, 400);
      } else if (reply === 'Bike Not Starting') {
        setStep('start_detail');
        setTimeout(() => {
          addBotMessage('Let me help with that. What happens when you try to start the bike?', START_DETAILS);
        }, 400);
      } else if (reply === 'Engine Noise') {
        setStep('engine_detail');
        setTimeout(() => {
          addBotMessage('Engine issues need attention! What kind of noise or symptom are you noticing?', ENGINE_DETAILS);
        }, 400);
      } else {
        setStep('other_detail');
        setTimeout(() => {
          addBotMessage("No worries! Please describe your issue briefly. I'll note it down for our technician.", undefined);
        }, 400);
      }
    } else if (
      ...
      setStep('duration');
      setTimeout(() => {
        addBotMessage('How long have you been experiencing this issue?', DURATION_OPTIONS);
      }, 400);
